...
const STAFF = ["Tuana", "Leyla", "Melike"];

const TODAY: Appt[] = [
  { time: "09:00", end: "09:45", client: "Sandra Köhler", service: "Maniküre + Shellac", staff: "Tuana", price: 38, status: "erschienen" },
  { time: "09:30", end: "11:00", client: "Aylin Demir", service: "Gel-Nägel Neu", staff: "Leyla", price: 59, status: "erschienen" },
  { time: "10:00", end: "11:30", client: "Jessica Bauer", service: "French & Babyboomer", staff: "Melike", price: 55, status: "bestätigt" },
  { time: "11:15", end: "12:00", client: "Petra Lang", service: "Maniküre & Naturnagel", staff: "Tuana", price: 28, status: "bestätigt" },
  { time: "13:00", end: "14:15", client: "Nicole Wagner", service: "Auffüllen & Refill", staff: "Leyla", price: 42, status: "bestätigt" },
  { time: "13:30", end: "15:00", client: "Melisa Yıldız", service: "Nail Art Vollset", staff: "Melike", price: 78, status: "offen" },
  { time: "15:00", end: "15:45", client: "Karin Hofmann", service: "Maniküre + Shellac", staff: "Tuana", price: 38, status: "bestätigt" },
  { time: "16:30", end: "18:00", client: "Tanja Roth", service: "Gel-Nägel Neu", staff: "Leyla", price: 59, status: "offen" },
];
...
export function TmNailsAdminPanel({ lead }: { lead: LeadProfile }) {
    ...
    <div className="tm-admin-panel">
      <aside className="tm-admin-sidebar">
        <div className="tm-admin-brand">
          <span className="tm-admin-logo">T<i>M</i> NAILS</span>
          <span className="tm-admin-brand-sub">Salonverwaltung</span>
        </div>
        <nav className="tm-admin-nav">
          {NAV.map((item) => (
            <button
              key={item.key}
              type="button"
              className={`tm-admin-navitem${view === item.key ? " is-active" : ""}${item.done ? "" : " is-wip"}`}
              onClick={() => setView(item.key)}
            >
              {item.icon}
              <span>{item.label}</span>
              {!item.done && <em className="tm-admin-dot" aria-label="In Entwicklung" />}
            </button>
          ))}
        </nav>
        <Link className="tm-admin-back" href={`/demo/${lead.slug}`}>
      ...
      <main className="tm-admin-main">
        <header className="tm-admin-topbar">
          ...
          <span className="tm-admin-tag">Fiktive Daten · nichts wird gespeichert</span>
        </header>

        <div className="tm-admin-content">
...
function StatusBadge({ status }: { status: Status }) {
  return <span className={`tm-admin-status s-${status}`}>{STATUS_ICON[status]} {status}</span>;
}
...
function Dashboard({ onJump }: { onJump: (v: ViewKey) => void }) {
  ...
  return (
    <>
      {open.length > 0 && (
        <button type="button" className="tm-admin-alert" onClick={() => onJump("calendar")}>
          <Clock size={16} />
          <span>{open.length} Termin(e) noch nicht bestätigt — Erinnerung per WhatsApp senden.</span>
        </button>
      )}
      <div className="tm-admin-tiles">
        {tiles.map((t) => (
          <article key={t.label} className={`tm-admin-tile tone-${t.tone}`}>
            <span className="tm-admin-tile-icon">{t.icon}</span>
            <strong>{t.value}</strong>
            <span className="tm-admin-tile-label">{t.label}</span>
          </article>
        ))}
      </div>

      <section className="tm-admin-block">
        <h2><CalendarDays size={17} /> Heutige Termine</h2>
        <div className="tm-admin-timeline">
          {TODAY.map((a) => (
            <article className={`tm-admin-slot${a.status === "abgesagt" ? " is-cancelled" : ""}`} key={a.time + a.client}>
              <span className="tm-admin-slot-time">{a.time}<small>{a.end}</small></span>
              <span className={`tm-admin-slot-bar staff-${STAFF.indexOf(a.staff)}`} />
              <span className="tm-admin-slot-body">
                <strong>{a.client}</strong>
                <small>{a.service} · {a.staff}</small>
              </span>
              <span className="tm-admin-slot-price">{a.price} €</span>
...
function Clients() {
  return (
    <section className="tm-admin-block">
      ...
      <div className="tm-admin-table cols-clients">
        <div className="tm-admin-row head">
          <span>Name</span><span>Besuche</span><span>Letzter</span><span>Favorit</span><span>Telefon</span>
        </div>
        {CLIENTS.map((c) => (
          <div className="tm-admin-row" key={c.name}>
            <span className="tm-admin-name">
              <i className="tm-admin-avatar">{c.name.charAt(0)}</i>
              {c.name}{c.vip && <em className="tm-admin-vip"><Star size={11} /> VIP</em>}
            </span>
            <span>{c.visits}×</span>
            <span>{c.last}</span>
            <span>{c.favorite}</span>
            <span className="tm-admin-phone"><Phone size={12} /> {c.phone}</span>
          </div>
        ))}
      </div>
    </section>
...
function Services() {
  return (
    <section className="tm-admin-block">
      ...
      <div className="tm-admin-services">
        {SERVICES.map((s) => (
          <article className="tm-admin-service-card" key={s.name}>
            <span className="tm-admin-service-icon"><Sparkles size={16} /></span>
            <div className="tm-admin-service-body">
              <h3>{s.name}{s.popular && <em className="tm-admin-popular">beliebt</em>}</h3>
              <p><Clock size={12} /> {s.duration}</p>
            </div>
            <span className="tm-admin-service-price">{s.price}</span>
          </article>
        ))}
      </div>
    </section>
...
function Stats() {
  ...
  return (
    <>
      <div className="tm-admin-tiles">
        <article className="tm-admin-tile tone-gold"><span className="tm-admin-tile-icon"><Euro size={16} /></span><strong>3.140 €</strong><span className="tm-admin-tile-label">Umsatz diese Woche</span></article>
        <article className="tm-admin-tile tone-ok"><span className="tm-admin-tile-icon"><CalendarDays size={16} /></span><strong>62</strong><span className="tm-admin-tile-label">Termine diese Woche</span></article>
        <article className="tm-admin-tile tone-neutral"><span className="tm-admin-tile-icon"><LayoutGrid size={16} /></span><strong>86 %</strong><span className="tm-admin-tile-label">Ø Auslastung</span></article>
        <article className="tm-admin-tile tone-warn"><span className="tm-admin-tile-icon"><UserRound size={16} /></span><strong>+7</strong><span className="tm-admin-tile-label">Neue Kundinnen</span></article>
      </div>
      <section className="tm-admin-block">
        <h2><TrendingUp size={17} /> Auslastung pro Tag</h2>
        <div className="tm-admin-bars">
          {WEEK.map((w) => (
            <div className="tm-admin-bar-col" key={w.day}>
              <span className="tm-admin-bar-val">{w.count}</span>
              <div className="tm-admin-bar-track">
                <div className="tm-admin-bar-fill" style={{ height: `${(w.count / max) * 100}%`, opacity: 0.45 + w.load * 0.55 }} />
              </div>
              <span className="tm-admin-bar-day">{w.day}</span>
            </div>
          ))}
        </div>
      </section>
...
function Wip({ title, copy }: { title: string; copy: string }) {
  return (
    <div className="tm-admin-wip">
      <span className="tm-admin-wip-badge">In Entwicklung</span>
      <h2>{title}</h2>
      <p>{copy}</p>
      <p className="tm-admin-wip-note">Dieses Modul ist Teil des geplanten Funktionsumfangs und noch nicht umgesetzt.</p>
    </div>
  );
}
